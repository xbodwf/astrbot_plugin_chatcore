...
AI 回复中途用户发新消息（无论这条消息是否命中普通回复概率）时：
...
群聊回复可能需要较长时间生成。生成期间到达的新消息不在首个请求快照中，
因此模型可能错过本应引用或 @ 的对象。ChatCore 提供可选的独立判断模型：

- 首段生成并发送后，立即异步调用 `providers.reply_decision.provider_id`；正常回复不等待该模型。
- 判断模型接收从触发消息到首段完成时的最新会话记录，因此包含生成期间已经到达的消息，以及首段文本。
- 模型只返回 JSON：`{"reply":"昵称或空字符串","at":"昵称或空字符串"}`。
- 判断结果作为后续分段的自动默认装饰；每个分段仍独立解析 `[[reply:昵称]]` / `[[at:昵称]]`，显式 marker 优先且可覆盖自动决策。
- 判断失败、超时或未配置 provider 时不添加任何自动装饰，不影响主回复。
- `reply_decision.enabled`、`timeout_seconds` 和 `prompt` 均可配置。该模型不是主聊天模型，避免延迟模型改变人格内容。
...
- **分析**：LLM 从样本中提炼三类产出并落 JSON：
  ...
  - 表达风格素材及 LLM 推断的含义（`?` 有歧义时保留来源例句）；
  - 该群表达风格的一句话总结。
- **注入**：对应群的 system prompt 追加「表达风格」块，包含句式偏好 + 表达素材（带例句）。
- **证据约束**：表达素材必须保存 `guessedMeaning`、`example`、`source`，缺任何一项不入库；注入时按当前对话相关性选少量条目，并可在 WebUI 查看原始来源。
- **共享**：`expression.share_across_groups` 默认开启，所有群可复用已审核的表达风格素材，但不共享聊天记录、记忆或画像；`shared_groups` 可额外指定来源。
- **人工维护**：WebUI 支持手动添加句式或表达素材，并填写 `guessedMeaning`、`example`、`source`；每条素材可启用/禁用，人工条目标记 `origin=manual`，不会被自动学习覆盖。
