...
class Main(Star):
    ...
    async def _build_system_prompt(
        ...
        rules = (
            ...
            "想回复某人的消息用 `[[reply:昵称]]`。"
            "③ 解析 `<message ...>内容</message>` 时，开始标签和结束标签仅用于标记"
            "消息边界，标签属性（如 uid、nickname、msg_id、time）仅是发送者元数据，"
            "不属于用户实际发送的信息；标签内部的全部内容才是用户消息，内部嵌套的"
            "标签及其内容也属于用户消息正文，不能把 XML 外壳本身当作用户说的话。"
        )
